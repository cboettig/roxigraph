# Authors and Citation

## Authors

- **Carl Boettiger**. Author, maintainer.
  [](https://orcid.org/0000-0002-1642-628X)

- **Oxigraph Contributors**. Copyright holder.  
  Oxigraph Rust library

- **Authors of the dependency Rust crates**. Author.  
  see inst/AUTHORS file

## Citation

Source:
[`DESCRIPTION`](https://github.com/cboettig/roxigraph/blob/main/DESCRIPTION)

Boettiger C, Authors of the dependency Rust crates (2026). *roxigraph:
'RDF' and 'SPARQL' for R using 'Oxigraph'*. R package version 0.1.1,
<https://github.com/cboettig/roxigraph>.

    @Manual{,
      title = {roxigraph: 'RDF' and 'SPARQL' for R using 'Oxigraph'},
      author = {Carl Boettiger and {Authors of the dependency Rust crates}},
      year = {2026},
      note = {R package version 0.1.1},
      url = {https://github.com/cboettig/roxigraph},
    }

## Additional details

    Authors of vendored cargo crates
     - aho-corasick 1.1.4: Andrew Gallant
     - bindgen 0.72.1: Jyun-Yan You, Emilio Cobos Álvarez, Nick Fitzgerald, The Servo project developers
     - bitflags 2.10.0: The Rust Project Developers
     - block-buffer 0.10.4: RustCrypto Developers
     - cc 1.2.55: Alex Crichton
     - cexpr 0.6.0: Jethro Beekman
     - cfg-if 1.0.4: Alex Crichton
     - clang-sys 1.8.1: Kyle Mayes
     - cpufeatures 0.2.17: RustCrypto Developers
     - crypto-common 0.1.7: RustCrypto Developers
     - dashmap 6.1.0: Acrimon
     - digest 0.10.7: RustCrypto Developers
     - either 1.15.0: bluss
     - extendr-api 0.8.1: andy-thomason, Thomas Down, Mossa Merhi Reimert, Josiah Parry, Claus O. Wilke, Hiroaki Yutani, Ilia A. Kosenkov, Michael Milton
     - extendr-ffi 0.8.1: andy-thomason, Thomas Down, Mossa Merhi Reimert, Josiah Parry, Claus O. Wilke, Hiroaki Yutani, Ilia A. Kosenkov, Michael Milton
     - extendr-macros 0.8.1: andy-thomason, Thomas Down, Mossa Merhi Reimert, Josiah Parry, Claus O. Wilke, Hiroaki Yutani, Ilia A. Kosenkov, Michael Milton
     - generic-array 0.14.7: Bartłomiej Kamiński, Aaron Trent
     - getrandom 0.3.4: The Rand Project Developers
     - glob 0.3.3: The Rust Project Developers
     - hashbrown 0.14.5: Amanieu d'Antras
     - hex 0.4.3: KokaKiwi
     - itertools 0.13.0: bluss
     - jobserver 0.1.34: Alex Crichton
     - json-event-parser 0.2.2: Tpt
     - libc 0.2.180: The Rust Project Developers
     - libloading 0.8.9: Simonas Kazlauskas
     - lock_api 0.4.14: Amanieu d'Antras
     - log 0.4.29: The Rust Project Developers
     - md-5 0.10.6: RustCrypto Developers
     - memchr 2.7.6: Andrew Gallant, bluss
     - minimal-lexical 0.2.1: Alex Huszagh
     - nom 7.1.3: contact@geoffroycouprie.com
     - once_cell 1.21.3: Aleksey Kladov
     - oxigraph 0.5.2: Tpt
     - oxilangtag 0.1.5: Tpt
     - oxiri 0.2.11: Tpt
     - oxjsonld 0.2.1: Tpt
     - oxrdf 0.3.1: Tpt
     - oxrdfio 0.2.1: Tpt
     - oxrdfxml 0.2.1: Tpt
     - oxrocksdb-sys 0.5.2: Tpt
     - oxsdatatypes 0.2.2: Tpt
     - oxttl 0.2.1: Tpt
     - parking_lot_core 0.9.12: Amanieu d'Antras
     - paste 1.0.15: David Tolnay
     - peg 0.8.5: Kevin Mehall
     - peg-macros 0.8.5: Kevin Mehall
     - peg-runtime 0.8.5: Kevin Mehall
     - ppv-lite86 0.2.21: The CryptoCorrosion Contributors
     - prettyplease 0.2.37: David Tolnay
     - proc-macro2 1.0.106: David Tolnay, Alex Crichton
     - quote 1.0.44: David Tolnay
     - rand 0.9.2: The Rand Project Developers, The Rust Project Developers
     - rand_chacha 0.9.0: The Rand Project Developers, The Rust Project Developers, The CryptoCorrosion Contributors
     - rand_core 0.9.5: The Rand Project Developers, The Rust Project Developers
     - redox_syscall 0.5.18: Jeremy Soller
     - regex 1.12.3: The Rust Project Developers, Andrew Gallant
     - regex-automata 0.4.14: The Rust Project Developers, Andrew Gallant
     - regex-syntax 0.8.9: The Rust Project Developers, Andrew Gallant
     - rustc-hash 2.1.1: The Rust Project Developers
     - scopeguard 1.2.0: bluss
     - serde 1.0.228: Erick Tryzelaar, David Tolnay
     - serde_core 1.0.228: Erick Tryzelaar, David Tolnay
     - serde_derive 1.0.228: Erick Tryzelaar, David Tolnay
     - sha1 0.10.6: RustCrypto Developers
     - sha2 0.10.9: RustCrypto Developers
     - shlex 1.3.0: comex, Fenhl, Adrian Taylor, Alex Touchet, Daniel Parks, Garrett Berg
     - siphasher 1.0.2: Frank Denis
     - smallvec 1.15.1: The Servo Project Developers
     - sparesults 0.3.1: Tpt
     - spareval 0.2.2: Tpt
     - spargebra 0.4.2: Tpt
     - sparopt 0.3.2: Tpt
     - syn 2.0.114: David Tolnay
     - thiserror 2.0.18: David Tolnay
     - thiserror-impl 2.0.18: David Tolnay
     - typenum 1.19.0: Paho Lurie-Gregg, Andre Bogus
     - unicode-ident 1.0.22: David Tolnay
     - version_check 0.9.5: Sergio Benitez
     - wit-bindgen 0.51.0: Alex Crichton
     - zerocopy 0.8.39: Joshua Liebow-Feeser, Jack Wrenn
     - zerocopy-derive 0.8.39: Joshua Liebow-Feeser, Jack Wrenn

    (This file was auto-generated from 'cargo metadata' on 2026-02-10)
